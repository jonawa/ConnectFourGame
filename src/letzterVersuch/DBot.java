	...
	public int getColumn() {
		...
		for (column = 0; column < board.length; column++) {
			...
			if (row != -1 && checkXInARow(column, row, 4, thisPlayer, board)) {
				if (debug)
					System.out.println("Found something that makes me win: " + (column + 1)); // debug
				return column;
			}
		}
		...
		for (column = 0; column < board.length; column++) {
			...
			if (row != -1 && checkXInARow(column, row, 4, enemyPlayer, board)) {
				if (debug)
					System.out.println("Found something that makes enemy win: " + (column + 1)); // debug
				return column;
			}
		}
		...
		for (column = 0; column < 7; column++) {
			...
			if (row != -1 && checkXInARow(column, row, 2, enemyPlayer, board)) {
				if (debug)
					System.out.println("Found something (maybe) valuable: " + (column + 1)); // debug
				possibleSolutions.add(column);
			}
		}
		...
		while (posSolu.hasNext()) {
			...
			if (nextRow <= topRow && checkXInARow(possibleColumn, nextRow, 4, enemyPlayer, board)) {
				posSolu.remove();
				if (debug)
					System.out.println("removed solutionzzz, left is: " + possibleSolutions);
			}
		}
		...
		for (int col = 0; col < 7; col++) {
			if (board[col][topRow].getPlayer() != -1)
				...
				if (nextRow <= topRow && checkXInARow(col, nextRow, 4, enemyPlayer, board))
					veryBadIdeas.add(col);
			}
		}
		if (debug)
		...
		if (veryBadIdeas.size() == 7) {
			...
			if (debug)
				System.out.println("Computer found too many bad ideas! Choosing:  " + (column + 1));
			return column;
		} else {
			do {
				column = grn.nextInt(4) + grn.nextInt(4);
		...
		if (debug)
			System.out.println("me " + thisPlayer + " Found some very bad ideas" + veryBadIdeas + ", but that should work: " + (column + 1));
		return column;
	}
